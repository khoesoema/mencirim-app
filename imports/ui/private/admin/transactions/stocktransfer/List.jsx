...
export function StockTransferLists(props) {
	...
	const [invoices, invoicesLoading] = useTracker(() => {
		let subs = Meteor.subscribe('stockTransfer.list', {
			page,
			searchText,
		...
		let data = StocksTransfersCollections.find(
			{
				$or: [
					{
						transferNumber: {
	...
	const [invoicesCount, invoicesCountLoading] = useTracker(() => {
		let subs = Meteor.subscribe('stockTransfer.countList', {
			searchText,
		});

		let data = Counts.get('stockTransfer.countList.' + searchText);
		return [data, !subs.ready()];
	}, [searchText]);
											...
											{invoices.map((item, index) => (
												<tr key={index}>
													...
													<td>
														...
														{item.destinationRackID && (
															<>
																<hr />
																Rak : <br />[
																{
																	item.destinationRackCode
																}
																]{' '}
																{
																	item.destinationRackName
																}{' '}
															</>
														)}
													</td>
