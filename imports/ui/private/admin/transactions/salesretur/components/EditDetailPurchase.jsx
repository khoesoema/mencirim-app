...
const EditDetailPurchase = (props) => { 
    ...
    const [penjualanDetailData, penjualanDetailDataLoading] = useTracker(() => {
		...
		if ( selectedID ) {
			let subs = Meteor.subscribe('penjualanreturdetail.getByID', { _id: selectedID });
			isLoading = !subs.ready();

			data = PenjualanReturDetailCollections.findOne({ _id: selectedID });
		}
    ...
    return (
        <>
            <Modal 
     		  ...
     		  <Modal.Header>
     		    <Modal.Title>
					{'Edit Item Penjualan Retur # ' + itemNo } 
				</Modal.Title>
     		  </Modal.Header>
			  <hr />
     		  <Modal.Body>
