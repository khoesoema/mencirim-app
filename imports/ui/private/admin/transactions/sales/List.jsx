...
export function SalesInvoicesLists() {
	...
	const [invoices, invoicesLoading] = useTracker(() => {
		let subs = Meteor.subscribe('penjualan.list', {
			page,
			searchText,
		...
		let data = PenjualanCollections.find(
			{
				noFaktur: {
					$regex: searchText,
					$options: 'i',
	...
	const [invoicesCount, invoicesCountLoading] = useTracker(() => {
		let subs = Meteor.subscribe('penjualan.countList', {
			searchText,
		});

		let data = Counts.get('penjualan.countList.' + searchText);
					...
					<Row>
						<Col>
							<IconButton 
								...
								icon={<PlusIcon />} 
								onClick={(e) => {navigate('/AddSalesInvoice');}}
							>Add New</IconButton>
						</Col>
						<Col>
						<InputGroup inside>
													...
													<td className="text-center"> 	
														{item.status === 'Pending' &&
															<IconButton
																...
																placement="right"
																onClick={() => {
																navigate('/ViewSalesInvoice/' + item._id);
																}}
													...
													{item.status === 'Pending'
														? 	<td style={{ textAlign: "center" }}>
																<a className ="fakeLink"
																	onClick={( e ) => {
																		navigate('/EditSalesInvoice/' + item._id);
																	}}
																>
																	<FaPencilAlt /> 
																</a>
																...
																<a className ="fakeLink"
																	onClick={() => {
																		...
																		setDeleteConfirmationDialogTitle('Hapus data Penjualan');
																		setDeleteConfirmationDialogContent(
																			'Anda akan menghapus data Penjualan ' +
																				'[' +
																				moment(item.tglFaktur).format('YYYY-MM-DD') +
																				'] ' +
																				item.noFaktur +
																				'. Semua data yang berhubungan dengan Penjualan ini juga akan dihapus. Data yang sudah dihapus, tidak dapat dikembalikan, apakah anda yakin?'
																		);
																	}}
																	>
																	<FaTrashAlt />
