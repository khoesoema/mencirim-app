...
export function CustomerLists(props) {
	...
	const [customers, customersLoading] = useTracker(() => {
		let subs = Meteor.subscribe('customers.list', {
			page,
			searchText,
	...
	const [customersCount, customersCountLoading] = useTracker(() => {
		let subs = Meteor.subscribe('customers.countList', { searchText });

		let data = Counts.get('customers.countList.' + searchText);
		return [data, !subs.ready()];
	}, [searchText]);

	useEffect(() => {
		setMaxPage(Math.ceil(customersCount / 20));
	}, [customersCount]);
												...
												<tr key={index}>
													...
													<td style={{ textAlign: "center" }}>
														<a className ="fakeLink"
															onClick={( e ) => {
																navigate('/EditCustomer/' + item._id);
															}}
														>
															<FaPencilAlt /> 
														</a>
