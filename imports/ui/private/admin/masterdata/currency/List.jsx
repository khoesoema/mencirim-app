...
export function CurrencyLists(props) {
	...
	const [currencies, currenciesLoading] = useTracker(() => {
		let subs = Meteor.subscribe('currencies.list', {
			page,
			searchText,
	...
	const [currenciesCount, currenciesCountLoading] = useTracker(() => {
		let subs = Meteor.subscribe('currencies.countList', { searchText });

		let data = Counts.get('currencies.countList.' + searchText);
		return [data, !subs.ready()];
	}, [searchText]);
					...
					<Row>
						<Col>
							<IconButton 
								...
								icon={<PlusIcon />} 
								onClick={(e) => {navigate('/AddCurrency');}}
							>Add New</IconButton>
						</Col>
						<Col>
						<InputGroup inside>
										...
										<>
											{currencies.map((item, index) => (
												<tr key={index}>
													...
													<td>{item.code}</td>
													<td>{item.name}</td>
													<td style={{ textAlign: "right" }} data-formatter ="NumFormatter">
														{item.kurs}</td>
													...
													<td style={{ textAlign: "center" }}>
														<a className ="fakeLink"
															onClick={( e ) => {
																navigate('/EditCurrency/' + item._id);
															}}
														>
															<FaPencilAlt /> 
														</a>
