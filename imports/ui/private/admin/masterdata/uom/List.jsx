...
export function UOMLists(props) {
	...
	const [uom, uomLoading] = useTracker(() => {
		let subs = Meteor.subscribe('uom.list', {
			page,
			searchText,
	...
	const [uomCount, uomCountLoading] = useTracker(() => {
		let subs = Meteor.subscribe('uom.countList', { searchText });

		let data = Counts.get('uom.countList.' + searchText);
		return [data, !subs.ready()];
	}, [searchText]);

	useEffect(() => {
		setMaxPage(Math.ceil(uomCount / 20));
	}, [uomCount]);
