...
export function PurchaseReturLists() {
	...
	const [invoices, invoicesLoading] = useTracker(() => {
		let subs = Meteor.subscribe('returpembelian.list', {
			page,
			searchText,
		...
		let data = ReturPembelianCollections.find(
			{
				noFaktur: {
					$regex: searchText,
					$options: 'i',
	...
	const [invoicesCount, invoicesCountLoading] = useTracker(() => {
		let subs = Meteor.subscribe('returpembelian.countList', {
			searchText,
		});

		let data = Counts.get('returpembelian.countList.' + searchText);
		return [data, !subs.ready()];
	}, [searchText]);
					...
					<Row>
						<Col>
							<IconButton 
								...
								icon={<PlusIcon />} 
								onClick={(e) => {navigate('/AddPurchaseRetur');}}
							>Add New</IconButton>
						</Col>
						<Col>
						<InputGroup inside>
													...
													<td className="text-center"> 	
														{item.status === 'Pending' &&
															<IconButton
																...
																placement="right"
																onClick={() => {
																navigate('/ViewPurchaseRetur/' + item._id);
																}}
													...
													{item.status === 'Pending'
														? 	<td style={{ textAlign: "center" }}>
																<a className ="fakeLink"
																	onClick={( e ) => {
																		navigate('/EditPurchaseRetur/' + item._id);
																	}}
																>
																	<FaPencilAlt /> 
																</a>
																...
																<a className ="fakeLink"
																	onClick={() => {
																		...
																		setDeleteConfirmationDialogTitle('Hapus data Retur Pembelian');
																		setDeleteConfirmationDialogContent(
																			'Anda akan menghapus data Retur Pembelian ' +
																				'[' +
																				moment(item.tglFaktur).format('YYYY-MM-DD') +
																				'] ' +
																				item.noFaktur +
																				'. Semua data yang berhubungan dengan Retur Pembelian ini juga akan dihapus. Data yang sudah dihapus, tidak dapat dikembalikan, apakah anda yakin?'
																		);
																	}}
																	>
																	<FaTrashAlt />
