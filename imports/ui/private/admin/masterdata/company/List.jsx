...
export function CompanyLists(props) {
	...
	const [companies, companiesLoading] = useTracker(() => {
		let subs = Meteor.subscribe('companies.list', {
			page,
			searchText,
	...
	const [companiesCount, companiesCountLoading] = useTracker(() => {
		let subs = Meteor.subscribe('companies.countList', { searchText });

		let data = Counts.get('companies.countList.' + searchText);
		return [data, !subs.ready()];
	}, [searchText]);

	useEffect(() => {
		setMaxPage(Math.ceil(companiesCount / 20));
	}, [companiesCount]);
