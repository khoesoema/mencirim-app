...
export function ListProvinsi(props) {
	...
	const [states, statesLoading] = useTracker(() => {
		let subs = Meteor.subscribe('states.list', {
			page,
			searchText,
	...
	const [statesCount, statesCountLoading] = useTracker(() => {
		let subs = Meteor.subscribe('states.countList', { searchText });

		let data = Counts.get('states.countList.' + searchText);
		return [data, !subs.ready()];
	}, [searchText]);

	useEffect(() => {
		setMaxPage(Math.ceil(statesCount / 20));
	}, [statesCount]);
					...
					<Row>
						<Col>
							<IconButton 
								...
								icon={<PlusIcon />} 
								onClick={(e) => {navigate('/AddProvinsi');}}
							>Add New</IconButton>
						</Col>
						<Col>
						<InputGroup inside>
												...
												<tr key={index}>
													...
													<td style={{ textAlign: "center" }}>
														<a className ="fakeLink"
															onClick={( e ) => {
																navigate('/EditProvinsi/' + item._id);
															}}
														>
															<FaPencilAlt /> 
														</a>
