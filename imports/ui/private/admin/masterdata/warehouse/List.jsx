...
export function WarehouseLists(props) {
	...
	const [warehouses, warehousesLoading] = useTracker(() => {
		let subs = Meteor.subscribe('warehouses.list', {
			page,
			searchText,
	...
	const [warehousesCount, warehousesCountLoading] = useTracker(() => {
		let subs = Meteor.subscribe('warehouses.countList', { searchText });

		let data = Counts.get('warehouses.countList.' + searchText);
		return [data, !subs.ready()];
	}, [searchText]);

	useEffect(() => {
		setMaxPage(Math.ceil(warehousesCount / 20));
	}, [warehousesCount]);
