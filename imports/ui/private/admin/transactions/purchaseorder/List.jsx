...
export function PurchaseOrderLists() {
	...
	const [invoices, invoicesLoading] = useTracker(() => {
		let subs = Meteor.subscribe('orderpembelian.list', {
			page,
			searchText,
		...
		let data = OrderPembelianCollections.find(
			{
				noFaktur: {
					$regex: searchText,
					$options: 'i',
	...
	const [invoicesCount, invoicesCountLoading] = useTracker(() => {
		let subs = Meteor.subscribe('orderpembelian.countList', {
			searchText,
		});

		let data = Counts.get('orderpembelian.countList.' + searchText);
					...
					<Row>
						<Col>
							<IconButton 
								...
								icon={<PlusIcon />} 
								onClick={(e) => {navigate('/AddPurchaseOrder');}}
							>Add New</IconButton>
						</Col>
						<Col>
						<InputGroup inside>
												...
												<tr key={index}>
													...
													<td style={{ textAlign: "center" }}>
														<a className ="fakeLink"
															onClick={( e ) => {
																navigate('/EditPurchaseOrder/' + item._id);
															}}
														>
															<FaPencilAlt /> 
														</a>
														...
														<a className ="fakeLink"
															onClick={() => {
																...
																setDeleteConfirmationDialogTitle('Hapus data OrderPembelian');
																setDeleteConfirmationDialogContent(
																	'Anda akan menghapus data OrderPembelian ' +
																		'[' +
																		moment(item.tglFaktur).format('YYYY-MM-DD') +
																		'] ' +
																		item.noFaktur +
																		'. Semua data yang berhubungan dengan OrderPembelian ini juga akan dihapus. Data yang sudah dihapus, tidak dapat dikembalikan, apakah anda yakin?'
																);
															}}
															>
															<FaTrashAlt />
