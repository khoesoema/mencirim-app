...
export function CategoryLists(props) {
	...
	const [categories, categoriesLoading] = useTracker(() => {
		let subs = Meteor.subscribe('categories.list', {
			page,
			searchText,
	...
	const [categoriesCount, categoriesCountLoading] = useTracker(() => {
		let subs = Meteor.subscribe('categories.countList', { searchText });

		let data = Counts.get('categories.countList.' + searchText);
		return [data, !subs.ready()];
	}, [searchText]);

	useEffect(() => {
		setMaxPage(Math.ceil(categoriesCount / 20));
	}, [categoriesCount]);
