...
export function ListKota(props) {
	...
	const [cities, citiesLoading] = useTracker(() => {
		let subs = Meteor.subscribe('cities.list', {
			page,
			searchText,
	...
	const [citiesCount, citiesCountLoading] = useTracker(() => {
		let subs = Meteor.subscribe('cities.countList', { searchText });

		let data = Counts.get('cities.countList.' + searchText);
		return [data, !subs.ready()];
	}, [searchText]);

	useEffect(() => {
		setMaxPage(Math.ceil(citiesCount / 20));
	}, [citiesCount]);
