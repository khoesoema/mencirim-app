...
const EditDetailPurchase = (props) => {
	...
	const [pembelianDetailData, pembelianDetailDataLoading] = useTracker(() => {
		...
		if (selectedID) {
			let subs = Meteor.subscribe('pembeliandetail.getByID', { _id: selectedID });
			isLoading = !subs.ready();

			data = PembelianDetailCollections.findOne({ _id: selectedID });
		}
	...
	const calcHarga = (input) => {
		...
		if (jenisdisc1 === "Sub Discount") { subdisc = subdisc + (dischrg1 / ktskcl); }
		if (jenisdisc2 === "Sub Discount") { subdisc = subdisc + (dischrg2 / ktskcl); }
		if (jenisdisc3 === "Sub Discount") { subdisc = subdisc + (dischrg3 / ktskcl); }
		if (jenisdisc4 === "Sub Discount") { subdisc = subdisc + (dischrg4 / ktskcl); }
		if (jenisdisc5 === "Sub Discount") { subdisc = subdisc + (dischrg5 / ktskcl); }
