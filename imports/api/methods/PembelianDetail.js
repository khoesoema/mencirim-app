	...
	Meteor.methods({
		'pembeliandetail.add'(data) {
			...
			try {
				...
				let countExist = PembelianDetailCollections.find({
					noFaktur, itemNo
				}).count();
				...
				PembelianDetailCollections.insert(insertData);
		...
		'pembeliandetail.edit'(data) {
			...
			try {
				...
				let currData = PembelianDetailCollections.findOne({
					noFaktur, itemNo,
				});
				...
				PembelianDetailCollections.update({ _id }, { $set: updateData });
		...
		'pembeliandetail.delete'(data) {
			...
			try {
				...
				PembelianDetailCollections.remove({ _id });
