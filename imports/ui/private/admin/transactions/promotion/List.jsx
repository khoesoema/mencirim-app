...
export function PromotionLists() {
	...
	const [orderBy, setOrderBy] = useState('promoNo');
	const [order, setOrder] = useState(1);

	const [promotions, promotionsLoading] = useTracker(() => {
		let subs = Meteor.subscribe('promotions.list', {
			page,
			searchText,
		...
		let data = PromotionsCollections.find(
			{
				promoNo: {
					$regex: searchText,
					$options: 'i',
	...
	const [promotionsCount, promotionsCountLoading] = useTracker(() => {
		let subs = Meteor.subscribe('promotions.countList', {
			searchText,
		});

		let data = Counts.get('promotions.countList.' + searchText);
		return [data, !subs.ready()];
	}, [searchText]);

	useEffect(() => {
		setMaxPage(Math.ceil(promotionsCount / 20));
	}, [promotionsCount]);
					...
					<Row>
						<Col>
							<IconButton 
								...
								icon={<PlusIcon />} 
								onClick={(e) => {navigate('/AddPromotion');}}
							>Add New</IconButton>
						</Col>
						<Col>
						<InputGroup inside>
												...
												<tr key={index}>
													...
													<td style={{ textAlign: "center" }}>
														<a className ="fakeLink"
															onClick={( e ) => {
																navigate('/EditPromotion/' + item._id);
															}}
														>
															<FaPencilAlt /> 
														</a>
														...
														<a className ="fakeLink"
															...
															onClick={() => {
																...
																setPromoNo(item.promoNo);
																setDeleteConfirmationDialogOpen(true);
																setDeleteConfirmationDialogTitle('Hapus data Promotions');
																setDeleteConfirmationDialogContent(
																	'Anda akan menghapus data Promotions ' +
																		'[' +
																		moment(item.tglFaktur).format('YYYY-MM-DD') +
																		'] ' +
																		item.promoNo +
																		'. Semua data yang berhubungan dengan Promotions ini juga akan dihapus. Data yang sudah dihapus, tidak dapat dikembalikan, apakah anda yakin?'
																);
															}}
															>
															<FaTrashAlt />
