	...
	Meteor.methods({
		'returpembeliandetail.add'(data) {
			...
			try {
				...
				let countExist = ReturPembelianDetailCollections.find({
					noFaktur, itemNo
				}).count();
				...
				ReturPembelianDetailCollections.insert(insertData);
				...
				let errorCode = addErrorLog(
					...
					this,
					'BACKEND',
					'method.returpembeliandetail.add',
					tryErr.message
				);
		...
		'returpembeliandetail.edit'(data) {
			let returnData = {
				code: 200,
				title: '',
				message: '',
			...
			try {
				...
				let currData = ReturPembelianDetailCollections.findOne({
					noFaktur, itemNo,
				});
				...
				ReturPembelianDetailCollections.update({ _id }, { $set: updateData });
		...
		'returpembeliandetail.delete'(data) {
			let returnData = {
				code: 200,
				title: '',
				message: '',
			...
			try {
				...
				ReturPembelianDetailCollections.remove({ _id });
				...
				let errorCode = addErrorLog(
					...
					this,
					'BACKEND',
					'method.returpembeliandetail.delete',
					tryErr.message
				);
				throw new Meteor.Error(
					'Unexpected Error',
