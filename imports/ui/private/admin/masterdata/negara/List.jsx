...
export function ListNegara(props) {
	...
	const [countries, countriesLoading] = useTracker(() => {
		let subs = Meteor.subscribe('countries.list', {
			page,
			searchText,
	...
	const [countriesCount, countriesCountLoading] = useTracker(() => {
		let subs = Meteor.subscribe('countries.countList', { searchText });

		let data = Counts.get('countries.countList.' + searchText);
		return [data, !subs.ready()];
	}, [searchText]);

	useEffect(() => {
		setMaxPage(Math.ceil(countriesCount / 20));
	}, [countriesCount]);
