	...
	Meteor.methods({
		'pembelian.add'(data) {
			...
			try {
				...
				let tglJatuhTempo = data.tglJatuhTempo;
				...
				let countExist = PembelianCollections.find({
					noFaktur
				}).count();
				...
				PembelianCollections.insert(insertData);
		...
		'pembelian.edit'(data) {
			...
			try {
				...
				let currData = PembelianCollections.findOne({
					noFaktur
				});
				...
				PembelianCollections.update({ _id }, { $set: updateData });
		...
		'pembelian.delete'(data) {
			...
			try {
				...
				PembelianCollections.remove({ _id });
		...
		'pembelian.edit.reject'(data) {
			...
			try {
				...
				let detail = PembelianDetailCollections.find({noFaktur});

				detail.map((item) => {
					PembelianDetailCollections.update({_id: item._id}, { $set: { tglFaktur: currData.tglFaktur, status: 0 } });	
				});
		...
		'pembelian.edit.approve'(data) {
			...
			try {
				...
				detail.map((item) => {
					PembelianDetailCollections.update({_id: item._id}, { $set: { tglFaktur: currData.tglFaktur, available: item.ktsKecil, status: 1 } });
