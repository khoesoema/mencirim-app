...
export function SalesReturInvoicesLists() {
	...
	const [invoices, invoicesLoading] = useTracker(() => {
		let subs = Meteor.subscribe('penjualanretur.list', {
			page,
			searchText,
		...
		let data = PenjualanReturCollections.find(
			{
				noFaktur: {
					$regex: searchText,
					$options: 'i',
		...
		data.map((item, index) => {
			let dataCustomer = {};
			let code = item.customerID;

			if (code) {
				let subs2 = Meteor.subscribe('customers.getByCode', { code  });
				if(subs2.ready()){
					dataCustomer = CustomersCollections.findOne({ code });
				}	
			}	

			databaru[index] = {
				...item,
				customerCode: dataCustomer.code,
	...
	const [invoicesCount, invoicesCountLoading] = useTracker(() => {
		let subs = Meteor.subscribe('penjualanretur.countList', {
			searchText,
		});

		let data = Counts.get('penjualanretur.countList.' + searchText);
					...
					<Row>
						<Col>
							<IconButton 
								...
								icon={<PlusIcon />} 
								onClick={(e) => {navigate('/AddSalesReturInvoice');}}
							>Add New</IconButton>
						</Col>
						<Col>
						<InputGroup inside>
										...
										<>
											{invoices.map((item, index) => (
												<tr key={index}>
													...
													<td> { item.customerID ? ('[' + item.customerCode + '] ' + item.customerName ) : '' } </td>
													...
													<td className="text-center"> 	
														{item.status === 'Pending' &&
															<IconButton
																...
																placement="right"
																onClick={() => {
																navigate('/ViewSalesReturInvoice/' + item._id);
																}}
													...
													{item.status === 'Pending'
														? 	<td style={{ textAlign: "center" }}>
																<a className ="fakeLink"
																	onClick={( e ) => {
																		navigate('/EditSalesReturInvoice/' + item._id);
																	}}
																>
																	<FaPencilAlt /> 
																</a>
																...
																<a className ="fakeLink"
																	onClick={() => {
																		...
																		setDeleteConfirmationDialogTitle('Hapus data Penjualan Retur');
																		setDeleteConfirmationDialogContent(
																			'Anda akan menghapus data Penjualan Retur' +
																				'[' +
																				moment(item.tglFaktur).format('YYYY-MM-DD') +
																				'] ' +
																				item.noFaktur +
																				'. Semua data yang berhubungan dengan Penjualan Retur ini juga akan dihapus. Data yang sudah dihapus, tidak dapat dikembalikan, apakah anda yakin?'
																		);
																	}}
																	>
																	<FaTrashAlt />
