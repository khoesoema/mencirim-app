...
export function SupplierLists(props) {
	...
	const [vendors, vendorsLoading] = useTracker(() => {
		let subs = Meteor.subscribe('vendors.list', {
			page,
			searchText,
	...
	const [vendorsCount, vendorsCountLoading] = useTracker(() => {
		let subs = Meteor.subscribe('vendors.countList', { searchText });

		let data = Counts.get('vendors.countList.' + searchText);
		return [data, !subs.ready()];
	}, [searchText]);

	useEffect(() => {
		setMaxPage(Math.ceil(vendorsCount / 20));
	}, [vendorsCount]);
		...
		<>
			<div className="mainContainerRoot">
				...
				<div className="mainContent">
					...
					<Row>
						<Col>
							<IconButton 
								...
								icon={<PlusIcon />} 
								onClick={(e) => {navigate('/AddSupplier');}}
							>Add New</IconButton>
						</Col>
						<Col>
						<InputGroup inside>
					...
					<hr />
					<Table responsive striped bordered hover className="tbl-supplier">
						<thead>
							<tr>
